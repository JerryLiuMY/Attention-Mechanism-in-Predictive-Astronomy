...
from utils.phased_lstm_layer import PhasedLSTM
np.random.seed(1)
...
class BasicLSTM:
    ...
    def one_step_prediction(self, train_X, cross_X, test_X, mag_scaler):
        # Load Model
        if self.phased:
            lstm_model = load_model(self.phased_lstm_model_path, custom_objects={'PhasedLSTM': PhasedLSTM})
        else:
            lstm_model = load_model(self.standard_lstm_model_path, custom_objects={'PhasedLSTM': PhasedLSTM})
