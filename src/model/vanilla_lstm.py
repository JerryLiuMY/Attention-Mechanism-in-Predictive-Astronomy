...
class RunVanillaLSTM:
    # The model is now trained individually for each sample, so we feed in the crts_id for now
    def __init__(self, hidden_dim, epochs, batch_size, n_walkers, implementation, **kwargs):
        # Configuration
    ...
    def build_model(self, X_train, y_train):
        ...
        elif self.implementation == 'dropout':
    ...
    def recursive(self, t_pred, X, mag_scaler, delta_t_scaler):
        """Predicting the intensity recursively using a sliding window"""
        # print(t_pred)
